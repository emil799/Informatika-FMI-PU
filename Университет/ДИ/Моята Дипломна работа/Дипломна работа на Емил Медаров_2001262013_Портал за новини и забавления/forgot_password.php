...
// Проверка дали формата за имейл е изпратена
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email'])) {
    // Вземете имейла от формата
    $email = $_POST['email'];

    try {
        // Примерна връзка с базата данни
        $pdo = new PDO('mysql:host=localhost;dbname=daily_Brief', 'root', '');
        ...
        if ($user) {
            // Запазване на userId в сесията и показване на форма за въвеждане на нова парола
        ...
        $message = 'Грешка при връзка с базата данни: ' . $e->getMessage();
    }
}

// Ако потребителят е попълнил новата парола
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['new_password']) && isset($_SESSION['userId'])) {
    // Вземете новата парола от формата
    ...
    try {
        ...
        // Обновяване на паролата в базата данни
        $stmt = $pdo->prepare('UPDATE Users SET user_password = :password WHERE user_id = :userId');
        $stmt->execute(['password' => password_hash($newPassword, PASSWORD_DEFAULT), 'userId' => $userId]);

        // Извеждане на съобщение за успешна промяна на паролата
        $message = "Паролата е успешно променена.";
        // Изчистване на сесията след успешна промяна на паролата
...
<nav>
    ...
    <div class="form-container">
        ...
        <?php if (!isset($showResetForm)) : ?>
            <form id="form" action="forgot_password.php" method="post">
        ...
        <?php else : ?>
            <form id="form" action="forgot_password.php" method="post">
                ...
                <a href="login.php" class="back-to-login">Върни се към вход</a>
