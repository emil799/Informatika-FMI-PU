...
$user_id = $_SESSION['user']['user_id']; // ID-то на текущия потребител
    ...
    if (isset($_POST["edit_post_id"]) && isset($_POST["category"])) {
        ...
        $types = '';
            ...
            if (!empty($updateColumns)) {
                $sql = "UPDATE $table SET " . implode(", ", $updateColumns) . " WHERE $idColumn = ? AND user_id = $user_id";
...
if ($selectedCategory) {
    ...
    switch ($selectedCategory) {
        case 'България':
            ...
            case 'Бизнес':
                ...
                case 'Забавления':
                    ...
                    if ($table) {
                        $sql = "SELECT $idColumn, $titleColumn, $descriptionColumn, $photoColumn, date_published FROM $table 
        ...
        $count_sql = "SELECT COUNT(*) as total FROM $table WHERE user_id = $user_id";
        ...
        if ($result->num_rows > 0) {
            $postsHTML .= "<form class='edit-post' action='edit-client-post.php' method='get' enctype='multipart/form-data'>";
    ...
    if ($selectedPostId && $table && $idColumn && $titleColumn && $descriptionColumn && $photoColumn && $typeColumn) {
        $sql = "SELECT $idColumn, $titleColumn, $descriptionColumn, $photoColumn, $typeColumn FROM $table WHERE $idColumn = ? AND user_id = $user_id";
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
            die('Prepare failed: ' . htmlspecialchars($conn->error ?? 'Unknown error'));
        }
...
<nav>
    ...
    <div class="form-container">
        ...
        <form action="edit-client-post.php" method="get">
...
<?php if ($selectedCategory): ?>
    ...
    <?php if ($editPost) { ?>
        <div class="edit-post-form">
            ...
            <form action="edit-client-post.php" method="post" enctype="multipart/form-data">
